...
function check_if_user_own_product($product_id)
{
    ...
    if (is_user_logged_in()) {
        ...
        foreach($translations as $lang=>$translation) {
            $translationIds[] = $translation->element_id;
        }
        foreach($translationIds as $item) {
            if (wc_customer_bought_product($current_user->user_email, $current_user->ID, $item) == 'yes') {
                $owned = true;
            }
        }
        if ($owned == true) {
...
function get_products_purchased_by_user()
{
    if (is_user_logged_in()) {
        ...
        if ($orders) {
            ...
            $appropriateLang = array();
            foreach($product_ids as $one) {
                $appropriateLang[] = wpml_object_id_filter($one);
            }
            return $appropriateLang;
        }
    }
    return false;
}
